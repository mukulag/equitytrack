        ...
        <details key={t.id} className="glass-card rounded-lg border p-3">
          <summary className="flex items-center justify-between cursor-pointer">
            <div className="flex items-baseline gap-3">
              ...
              <div className="text-sm text-muted-foreground">{t.tradeType}</div>
            </div>

            <div className="text-right">
              <div className="font-semibold">
                {t.bookedProfit > 0 ? '+' : ''}
                {formatCurrency(t.bookedProfit)}
              </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
            <div className="truncate"><strong>Entry:</strong> {formatCurrency(t.entryPrice)}</div>
            <div className="truncate"><strong>CMP:</strong> {t.currentPrice ? formatCurrency(t.currentPrice) : '—'}</div>
